...
select opt in "${options[@]}"; do
    case $opt in
    "Private AKS Cluster")
        parameters="./azuredeploy.private.parameters.json"
        break
        ;;
    "Public AKS Cluster")
        parameters="./azuredeploy.public.parameters.json"
        break
        ;;
    "Quit")
        exit
...
aksResourceGroupName="ABCDS-Kube-Dev"
location="centralus"
...
aksExtensions=("PodSecurityPolicyPreview" "AKS-NATGatewayPreview" "RunCommandPreview" "EnablePodIdentityPreview " "UserAssignedIdentityPreview" "EnablePrivateClusterPublicFQDN" "PodSubnetPreview")
ok=0
...
# Add Application Gateway Ingress Controller addon
...
if [[ -n $applicationGatewayId ]]; then
    echo "Resource id of the [$applicationGatewayName] application gateway successfully retrieved"
    echo "Registering AGIC addon on the [$aksName] AKS cluster..."
    ...
    if [[ $? == 0 ]]; then
        echo "AGIC addon registration on the [$aksName] AKS cluster successfully completed"
    else
        echo "Failed to register the AGIC addon on the [$aksName] AKS cluster"
    fi
else
    echo "Failed to retrieve the resource id of the [$applicationGatewayName] application gateway"
fi
